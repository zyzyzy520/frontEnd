...
const { modelLogin, modelCompare, modelReg } = require('../Per/userModel')
const { compareSync } = require('bcrypt');
const jwt = require('jsonwebtoken');
// {TOKEN_KEY: TOKEN_KEY} = {TOKEN_KEY: TOKEN_KEY}, TOKEN_KEY属性名匹配，然后值给到TOKEN_KEY中
...
module.exports.serviceLogin = async function (userInfo) {
    ...
    if (flag) {
        ...
        const token = jwt.sign(
            ...
            TOKEN_KEY, //密钥字符串，用于加密，可以随便给
            { expiresIn: 20 } //设置token的有效期，单位若没给就是s
        );
        return {
            message: '登录成功！',
            status: 1,
